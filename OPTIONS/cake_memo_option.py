...
class CakeMemoOptionWidget(QWidget):
    ...
    def _handle_cake_save_and_close(self, filepath: str) -> None:
        ...
        try:
            ...
            self._log("Generating automated Outlook draft...")
            try:
                create_cake_memo_outlook_draft(dest_path, memo_data)
                self._log(f"Outlook draft created and displayed for Room {room_num}.", "SUCCESS")
                ...
                self._log(f"Outlook draft generation failed: {mail_err}", "WARNING")
                QMessageBox.warning(
                    self,
