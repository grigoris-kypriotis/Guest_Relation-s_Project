...
from MODULES.analytics_registry import CHART_REGISTRY, ChartSpec


def _resolve_todays_inhouse_backup():
    if not os.path.exists(DATA_BACKUP_DIR):
        return None
    candidates = [os.path.join(DATA_BACKUP_DIR, f) for f in os.listdir(DATA_BACKUP_DIR)]
...
class ResortStatsDialog(QDialog):
    ...
    def _load_manifest_data(self):
        ...
        if self.manifest_path and os.path.exists(self.manifest_path) and pd is not None:
            try:
    ...
    def _init_ui(self) -> None:
        ...
        manifest_name = os.path.basename(self.manifest_path) if self.manifest_path else "master_state.json"
        sub_title = QLabel(f"Integrated dataset: {os.path.basename(self.hotel_json_path)} | Manifest: {manifest_name}")
...
class StatsWidget(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        try:
            ...
            pass
        self.chart_cards: Dict[str, ChartCardWidget] = {}
        self.chart_figures: Dict[str, Figure] = {}
        self.chart_canvases: Dict[str, NonScrollableFigureCanvas] = {}
        self._init_ui()
    ...
    def _init_ui(self) -> None:
        ...
        """)
        self.btn_refresh.clicked.connect(self._on_refresh_stats_clicked)
        ...
        """)
        self.btn_clear_trace.clicked.connect(self._on_clear_trace_file)
        ...
        lens_layout.setContentsMargins(6, 4, 6, 4)
        lens_layout.setSpacing(6)

        lbl_lens = QLabel("🎯 OPERATIONAL LENS:")
        lbl_lens.setStyleSheet("font-weight: 800; color: #1E293B; font-size: 10.5px;")
        lens_layout.addWidget(lbl_lens)

        self.lens_buttons = {}
        lenses = [
            (LENS_ALL, "🌐 All"),
            (LENS_ROOM_STAY, "🛏️ Room & Stay"),
            (LENS_GR_TRACES, "🤝 GR Traces"),
            (LENS_DIETARY, "🥗 Dietary"),
        ]
        ...
        sec_charts = self._create_section_container(
            ...
            "High-DPI modular analytical charts: market mix, stay duration, upgrade/downgrade tracker, trace categories, complaint tags, dietary alerts, repeat rooms, spatial density, trace sub-categories, correlation diagnostics, normalized density, and friction index."
        )
        ...
        for spec in CHART_REGISTRY:
            card = ChartCardWidget(title=spec.title, subtitle=spec.subtitle, icon=spec.icon, min_canvas_height=300)
            card.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Minimum)
            card.setMinimumWidth(0)
            fig = Figure(figsize=(7.5, 3.0), dpi=100, facecolor="#FFFFFF")
            ...
            safe_title = spec.title.encode("ascii", "replace").decode("ascii")
            print(f"[StatsWidget] Canvas instance #{StatsWidget._canvas_creation_count} initialized (chart {spec.key}: {safe_title})")

            self.chart_cards[spec.key] = card
            self.chart_figures[spec.key] = fig
            self.chart_canvases[spec.key] = canvas
            chart_inner.addWidget(card)

        # Backward compatibility aliases for existing tests
        self.figure = self.chart_figures["tour_operator_mix"]
        self.canvas = self.chart_canvases["tour_operator_mix"]
        ...
        bk_grid.addWidget(self.box_nat, 0, 0)
        bk_grid.addWidget(self.box_rooms, 0, 1)
        bk_grid.addWidget(self.box_docs, 0, 2)
    ...
    def _create_kpi_card(self, title: str, main_stat: str, sub_stat: str, accent_color: str) -> QFrame:
        ...
        lbl_t = QLabel(title)
        lbl_t.setStyleSheet(f"font-size: 10px; font-weight: bold; color: {accent_color}; text-transform: uppercase;")
        lbl_t.setWordWrap(True)
        card.lbl_main = QLabel(main_stat)
        card.lbl_main.setStyleSheet("font-size: 15px; font-weight: 800; color: #1E293B;")
        card.lbl_main.setWordWrap(True)
        card.lbl_sub = QLabel(sub_stat)
        card.lbl_sub.setStyleSheet("font-size: 9.5px; color: #64748B;")
        card.lbl_sub.setWordWrap(True)
    ...
    def _on_lens_selected(self, active_lens: str) -> None:
        ...
        # Filter Charts
        for spec in CHART_REGISTRY:
            card = self.chart_cards.get(spec.key)
            if card:
                is_visible = (active_lens == LENS_ALL) or (active_lens in spec.lenses)
                card.setVisible(is_visible)

        # Filter / Emphasize KPI Cards
        if hasattr(self, "card_capacity"):
            kpi_lens_map = {
    ...
    def _fetch_all_stats_data(self) -> Dict[str, Any]:
        ...
        if self.active_trace_path and not os.path.exists(self.active_trace_path):
            self.active_trace_path = None

        if self.active_trace_path and os.path.exists(self.active_trace_path):
            traces = self.data_manager.parse_trace_file(self.active_trace_path)
        else:
            traces = []
        ...
        trace_analytics = compute_visual_analytics_data(
            ...
            in_house_csv_path=_resolve_todays_inhouse_backup()
        )
    ...
    def _on_refresh_stats_clicked(self) -> None:
        """Explicit handler for Refresh Stats button triggering fresh recalculation on demand."""
        self.btn_refresh.setEnabled(False)
        self.btn_refresh.setText("⏳ Refreshing...")
        self.lbl_status.setText("⏳ Refreshing analytics...")
        self.lbl_status.setStyleSheet("font-size: 11px; color: #D97706; font-weight: bold; padding-right: 6px;")
        from PyQt6.QtWidgets import QApplication
        QApplication.processEvents()

        try:
            self.refresh_stats(sync=True)
            self.lbl_status.setText(f"🟢 Live State (Refreshed at {datetime.now().strftime('%H:%M:%S')})")
            self.lbl_status.setStyleSheet("font-size: 11px; color: #065F46; font-weight: bold; padding-right: 6px;")
        except Exception as e:
            print(f"[StatsWidget] Refresh error: {e}")
            if os.environ.get("QT_QPA_PLATFORM") != "offscreen":
                QMessageBox.warning(self, "Refresh Error", f"Failed to refresh metrics:\n{e}")
        finally:
    ...
    def _render_charts(self, *args, **kwargs) -> None:
        ...
        if hasattr(self, "chart_figures"):
            for spec in CHART_REGISTRY:
                fig = self.chart_figures.get(spec.key)
                canvas = self.chart_canvases.get(spec.key)
                card = self.chart_cards.get(spec.key)
                if fig is None or canvas is None or card is None:
                    continue

                try:
                    render_func = getattr(TraceAnalyticsPlotEngine, spec.render_attr)
                    render_func(fig, trace_data)
                    canvas.draw_idle()
                except Exception as e:
                    print(f"[StatsWidget] Error rendering chart {spec.key}: {e}")

                try:
                    self._update_chart_badges(spec.key, trace_data, card)
                except Exception as e:
                    print(f"[StatsWidget] Error updating badges for {spec.key}: {e}")

    def _update_chart_badges(self, key: str, trace_data: Dict[str, Any], card: ChartCardWidget) -> None:
        if key == "tour_operator_mix":
            top_op = trace_data.get("tour_operator_mix", [{}])[0].get("operator", "N/A") if trace_data.get("tour_operator_mix") else "N/A"
            card.set_badges([("Top Market", top_op[:18], "#2563EB"), ("Markets Count", len(trace_data.get("tour_operator_mix", [])), "#059669")])
        elif key == "length_of_stay":
            stay_b = trace_data.get("length_of_stay_dist", {})
            peak_stay = max(stay_b, key=stay_b.get) if stay_b else "N/A"
            card.set_badges([("Peak Duration", peak_stay, "#3B82F6"), ("7-13 Nights", stay_b.get("7-13 nights", 0), "#059669")])
        elif key == "room_type_upgrade_downgrade":
        ...
        elif key == "trace_category_breakdown":
            cats = trace_data.get("trace_category_breakdown", [])
            top_cat = cats[0]["category"] if cats else "N/A"
            card.set_badges([("Total Traces", trace_data.get("total_traces", 0), "#4338CA"), ("Top Category", top_cat, "#6366F1")])
        elif key == "rcr_reason_tagging":
            rcr_tags = trace_data.get("rcr_reason_tagging", [])
            top_rcr = rcr_tags[0]["reason"] if rcr_tags else "N/A"
            card.set_badges([("Primary Complaint", top_rcr, "#DC2626"), ("Reason Categories", len(rcr_tags), "#D97706")])
        elif key == "allergy_dietary_frequency":
            ...
            card.set_badges([("Top Dietary Alert", top_allg, "#D97706"), ("Affected Guests", tot_allg, "#2563EB")])
        elif key == "repeat_issue_rooms":
            rep_rms = trace_data.get("repeat_issue_rooms", [])
            diff_g = sum(1 for r in rep_rms if r.get("guest_pattern") == "different_guests")
            same_g = sum(1 for r in rep_rms if r.get("guest_pattern") == "same_guest")
            badges = [("Problem Rooms (≥2)", len(rep_rms), "#DC2626" if rep_rms else "#059669")]
            if rep_rms:
                badges.append(("Diff Guests", diff_g, "#DC2626"))
                badges.append(("Same Guest", same_g, "#D97706"))
            card.set_badges(badges)
        elif key == "block_floor_density":
            density_matrix = trace_data.get("block_floor_complaint_density", {})
            blocks = density_matrix.get("blocks", [])
            card.set_badges([("Monitored Blocks", len(blocks), "#1E293B"), ("Floors per Block", 3, "#475569")])
        elif key == "trace_subcategory_breakdown":
            ...
            card.set_badges([("Total Traces", tot_sub, "#3B82F6"), ("Top Sub-Category", top_subcat, "#8B5CF6")])
        elif key == "trace_room_change_correlation":
            corr = trace_data.get("trace_room_change_correlation", {})
            rate = corr.get("prior_trace_rate", 0.0)
            card.set_badges([
                ("Prior Rate", f"{rate:.1f}%", "#3B82F6"),
                ("Prior Traces", corr.get("rooms_with_prior_traces", 0), "#3B82F6"),
                ("Zero Prior", corr.get("rooms_zero_prior_traces", 0), "#F59E0B"),
                ("Approved Moves", corr.get("formally_approved_room_changes", 0), "#10B981")
            ])
        elif key == "occupancy_normalized_issue_density":
            dens = trace_data.get("occupancy_normalized_issue_density", {}).get("block_densities", [])
            ...
            if valid_dens:
                highest_dens = max(valid_dens, key=lambda x: x["density_ratio"])
                highest_ratio = highest_dens.get("density_ratio", 0.0)
                highest_blk = highest_dens.get("block", "N/A")
                badges = [
                    ("Highest Density", f"Block {highest_blk} ({highest_ratio:.2f})", "#DC2626"),
                    ("Blocks Evaluated", len(dens), "#2563EB")
                ]
                if vacant_blocks:
                    badges.append(("Vacant Blocks", len(vacant_blocks), "#F59E0B"))
                card.set_badges(badges)
            elif vacant_blocks:
                card.set_badges([
                    ("Status", "All Blocks Vacant", "#F59E0B"),
                    ("Blocks Evaluated", len(dens), "#2563EB")
                ])
            else:
                card.set_badges([
                    ("Occupancy", "Manifest Required", "#64748B"),
                    ("Blocks Evaluated", len(dens), "#2563EB")
                ])
        elif key == "profile_friction_index":
            fric_info = trace_data.get("profile_friction_index", {})
            vip_rate = fric_info.get("vips", {}).get("friction_index", 0.0)
            rep_rate = fric_info.get("repeaters", {}).get("friction_index", 0.0)
            card.set_badges([
                ("VIP Friction", f"{vip_rate:.2f}", "#8B5CF6"),
                ("Repeater Friction", f"{rep_rate:.2f}", "#10B981")
            ])

    # -----------------------------------------------------------------
    # Trace File Handling & Room Mapping Callbacks
    # -----------------------------------------------------------------
    def _on_clear_trace_file(self) -> None:
        """Manually unbinds and clears the active trace file."""
        self.active_trace_path = None
        if hasattr(self, "lbl_trace_file"):
            self.lbl_trace_file.setText("No trace file loaded")
            self.lbl_trace_file.setStyleSheet("font-size: 11px; color: #94A3B8; font-style: italic;")
    ...
    def reset_traces(self, clear_file_path: bool = True) -> None:
        """Purges active trace references and refreshes the stats dashboard."""
        if clear_file_path:
            self.active_trace_path = None
        if hasattr(self, "lbl_trace_file"):
            self.lbl_trace_file.setText("No trace file loaded")
